      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <input name="nombre" value={form.nombre} onChange={handleChange} className="card rounded px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-yellow-300" />
          </div>
          <div>
            <label className="block text-xs font-semibold muted mb-1">Email</label>
            <input name="email" type="email" value={form.email} onChange={handleChange} className="card rounded px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-yellow-300" />
          </div>
          <div>
            <label className="block text-xs font-semibold muted mb-1">Rol</label>
            <select name="id_rol" value={form.id_rol} onChange={handleChange} className="card rounded px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-yellow-300">
          ...
          <div>
            <label className="block text-xs font-semibold muted mb-1">Nivel acceso</label>
            <select
              name="nivel_acceso"
              value={form.nivel_acceso}
              onChange={handleChange}
              disabled={usuario?.nivel_acceso === 0}
              className="card rounded px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-yellow-300 disabled:opacity-60 disabled:cursor-not-allowed"
            >
            ...
            {usuario?.nivel_acceso === 0 && (
              <p className="text-[11px] muted mt-1">Nivel 0 (Admin) no modificable aquí.</p>
            )}
            {usuario?.nivel_acceso !== 0 && (
              <p className="text-[11px] muted mt-1">Solo se puede asignar 1 (Supervisor) o 2 (Básico).</p>
            )}
          </div>
          <div className="md:col-span-2">
            <label className="block text-xs font-semibold muted mb-1">Nueva contraseña (para reset)</label>
            <input name="password" type="password" value={form.password} onChange={handleChange} placeholder="Ingresa la nueva contraseña del usuario" className="card rounded px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-yellow-300" />
            <div className="flex items-center justify-between mt-2">
              <p className="text-[11px] muted">Escribe la nueva contraseña y pulsa Resetear.</p>
              <button type="button" onClick={doResetPassword} disabled={loading} className="px-3 h-8 inline-flex items-center rounded-full text-[12px] font-medium card hover:shadow-sm disabled:opacity-50">{loading ? '...' : 'Resetear'}</button>
            </div>
