...
export default function ExportInstrumentosModal({ open, onClose, selectedIds = [], defaultFormat = 'csv', onExported }) {
  ...
  const doExport = async () => {
    try {
      setExporting(true);
      const { exportInstrumentos } = await import('../../api/instrumentos.js');
      const format = exportFormat;
      const ids = Array.isArray(selectedIds) && selectedIds.length > 0 ? selectedIds : [];
      const res = await exportInstrumentos({ ids, format });
      ...
      a.href = url;
      a.download = `instrumentos_export_${Date.now()}.${format}`;
      a.click();
  ...
  return (
    <Modal title="Exportar instrumentos" onClose={onClose}>
      <div className="min-w-[520px] space-y-5">
        <p className="text-sm muted">
          Exportar {selectedIds?.length > 0 ? `${selectedIds.length} seleccionado${selectedIds.length !== 1 ? 's' : ''}` : 'todos los instrumentos'} como:
        </p>
