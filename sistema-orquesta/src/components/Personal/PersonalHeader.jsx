...
export default function PersonalHeader({ onCreate, selected = [], onExport, onOpenActions }) {
  const count = selected.length;
  return (
    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
      <div className="space-y-1">
        <h1 className="text-2xl font-bold tracking-tight">Personal</h1>
        <p className="text-sm text-gray-500">Gestión de personal: datos, cargo, programa y carga horaria.</p>
      </div>
      <div className="flex flex-col sm:flex-row sm:items-center gap-3" aria-label="Acciones personal" role="group">
        {count > 0 && (
          <div className="flex items-center gap-2 card-90 backdrop-blur-sm rounded-full pl-3 pr-4 h-10 shadow-sm" role="status" aria-live="polite">
