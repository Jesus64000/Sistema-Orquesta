...
export default function ThemeToggle({ className = '' }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 px-3 py-1 rounded-full card text-app text-sm ${className}`}
    >
      ...
      <span className="hidden sm:inline">{displayMode === 'dark' ? 'Oscuro' : 'Claro'}</span>
    </button>
  );
}
